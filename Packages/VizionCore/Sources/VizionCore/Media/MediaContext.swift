...
public struct MediaContextItem: Sendable, Hashable {
  ...
  public init(role: AttachmentRole, isReady: Bool, name: String, description: String?, attrs: MediaAttributes?) {
    self.role = role
...
public enum MediaContext {
  ...
  public static let maxChars = 1_500
  ...
  static func summarize(_ attrs: MediaAttributes?) -> String? {
    ...
    var bits: [String] = []
    if let s = attrs.subject { bits.append(s) }
    if let s = attrs.style { bits.append("\(s) style") }
    if let m = attrs.mood { bits.append("\(m) mood") }
    if let p = attrs.palette, !p.isEmpty { bits.append("palette \(p.prefix(4).joined(separator: " "))") }
    if let w = attrs.width, let h = attrs.height { bits.append("\(w)×\(h)") }
    if let d = attrs.durationSec { bits.append("~\(Int(d.rounded()))s") }
  ...
  public static func build(_ items: [MediaContextItem]) -> [String] {
    ...
    for item in items {
      ...
      if blocks.count >= maxItems { break }
    }
    return blocks
  }
  ...
  public static func styleSnippet(_ attrs: MediaAttributes) -> String {
    var bits: [String] = []
    if let s = attrs.style { bits.append(s) }
    if let l = attrs.lighting { bits.append("\(l) lighting") }
    if let m = attrs.mood { bits.append("\(m) mood") }
    if let p = attrs.palette, !p.isEmpty { bits.append("palette \(p.prefix(6).joined(separator: " "))") }
