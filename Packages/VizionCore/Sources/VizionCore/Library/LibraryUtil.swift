...
public enum LibraryUtil {
  ...
  public static func relativeTime(_ date: Date, now: Date = Date(), calendar: Calendar = .current) -> String {
    ...
    if sec < 3600 { return "\(Swift.max(1, sec / 60)) min ago" }
    if sec < 86_400 { return "\(sec / 3600) hr ago" }
  ...
  public static func deriveTitle(_ input: String, max: Int = 60) -> String {
    ...
    if base.isEmpty { return "Untitled prompt" }
  ...
  public static func contentHash(input: String, output: String, mode: String, target: String) -> String {
