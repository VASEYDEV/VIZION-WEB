...
final class SupportTests: XCTestCase {
  func testPostgresDateParsesMicrosecondsAndOffsets() {
    let a = PostgresDate.parse("2026-08-15T11:30:00.123456+00:00")!
    XCTAssertEqual(a.timeIntervalSince1970, 1_786_793_400.123456, accuracy: 0.001)
    let b = PostgresDate.parse("2026-08-15T11:30:00Z")!
    XCTAssertEqual(b.timeIntervalSince1970, 1_786_793_400, accuracy: 0.001)
    let c = PostgresDate.parse("2026-08-15 13:30:00+02")!
    XCTAssertEqual(c.timeIntervalSince1970, 1_786_793_400, accuracy: 0.001)
    let d = PostgresDate.parse("2026-08-15T06:30:00.5-05:00")!
  ...
  func testPasswordRule() {
    ...
    XCTAssertEqual(PasswordRule.validate("123456789012"), "Add a lowercase letter and an uppercase letter.")
  ...
  func testDeepLinks() {
    XCTAssertEqual(DeepLink.parse(URL(string: "vizion://enhance?draft=hello%20world")!), .enhance(draft: "hello world"))
    XCTAssertEqual(DeepLink.parse(URL(string: "https://vizion-io.vercel.app/enhance?draft=x")!), .enhance(draft: "x"))
    XCTAssertEqual(DeepLink.parse(URL(string: "vizion://library")!), .library)
    XCTAssertEqual(
      DeepLink.parse(URL(string: "vizion://library/123e4567-e89b-12d3-a456-426614174000")!),
      .prompt(id: "123e4567-e89b-12d3-a456-426614174000")
    )
    XCTAssertEqual(DeepLink.parse(URL(string: "vizion://library/not-a-uuid")!), .library)
    XCTAssertEqual(DeepLink.parse(URL(string: "vizion://profile")!), .settings)
    let cb = URL(string: "vizion://auth/callback?code=abc")!
    XCTAssertEqual(DeepLink.parse(cb), .authCallback(cb))
    XCTAssertEqual(DeepLink.parse(URL(string: "vizion://auth/callback?error=access_denied")!), .authError("access_denied"))
    XCTAssertNil(DeepLink.parse(URL(string: "mailto:x@y.z")!))
    XCTAssertNil(DeepLink.parse(URL(string: "vizion://nope")!))
  }
  ...
  func testDraftParam() {
    ...
    XCTAssertEqual(DraftParam.resolve(String(repeating: "a", count: 8_001), currentDraft: ""), .none)
  }
  ...
  func testMediaHelpers() {
    ...
    XCTAssertEqual(MediaAnalysisRequest.dataURL(mime: "image/png", base64: "AAAA"), "data:image/png;base64,AAAA")
    XCTAssertEqual(MediaBudget.formatBytes(500), "500 B")
    XCTAssertEqual(MediaBudget.formatBytes(2_048), "2 KB")
  ...
  func testMediaContextBuilder() {
    ...
    let items = [
      MediaContextItem(role: .reference, isReady: true, name: "ref.png", description: "A red door.", attrs: nil),
      MediaContextItem(role: .reference, isReady: false, name: "pending.png", description: "x", attrs: nil),
      MediaContextItem(role: .style, isReady: true, name: "style.png", description: "y", attrs: nil),
      MediaContextItem(
  ...
  func testGenerationFormatters() {
    let attrs = MediaAttributes(subject: "lighthouse", palette: ["#0a1e28", "#3fd4e8"], mood: "moody", width: 1080, height: 1920)
    XCTAssertEqual(
    ...
    XCTAssertEqual(
      GenerationPrompt.build(base: "Ambient", attrs: MediaAttributes(mood: "calm", durationSec: 29.6), target: .audio),
      "Ambient Mood: calm. Duration: ~30s."
    )
    XCTAssertEqual(GenerationPrompt.nearestAspect(width: 2000, height: 1000), "16:9", "2.0 is nearer 16:9 (1.78) than 21:9 (2.33)")
