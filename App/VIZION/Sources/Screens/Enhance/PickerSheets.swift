...
struct TargetPickerSheet: View {
          ...
          Section {
            ...
            if auto.wrappedValue, let autoPreference {
              VStack(alignment: .leading, spacing: 6) {
                Text("Budget").vzCaps()
                VZSegmented(
                  options: AutoPreference.detents.compactMap { d in AutoPreference(rawValue: d.id).map { (id: $0, label: d.label) } },
                  selection: Binding(get: { Optional(autoPreference.wrappedValue) }, set: { if let v = $0 { autoPreference.wrappedValue = v } }),
          ...
          Section(group.developer.label) {
            ForEach(group.models) { model in
              Button {
                ...
                row(label: model.label, detail: model.hasThinkingDial ? "Thinking dial" : nil,
  ...
  private func row(label: String, detail: String?, selected: Bool, @ViewBuilder mark: () -> some View) -> some View {
    HStack(spacing: 12) {
      mark().frame(width: 22)
      ...
      Spacer()
      if selected { IconView(.check, size: 18).foregroundStyle(VZ.accent) }
    }
...
struct ThinkingDialSheet: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 14) {
      ...
      Text("How hard \(target.label) reasons before it answers. Auto sends nothing — the provider's own default applies.")
    ...
    case .google:
      ZStack {
        ForEach(Array(ProviderPaths.google.enumerated()), id: \.offset) { _, piece in
          SVGShape(commands: piece.commands, viewBox: CGSize(width: 24, height: 24)).fill(piece.color)
        }
      }
...
enum ProviderPaths {
  ...
  static let google: [Piece] = [
    (0x4285F4, "M23.52 12.27c0-.82-.07-1.6-.21-2.36H12v4.47h6.46a5.52 5.52 0 0 1-2.4 3.62v3h3.88c2.27-2.09 3.58-5.17 3.58-8.73Z"),
    (0x34A853, "M12 24c3.24 0 5.96-1.08 7.94-2.92l-3.88-3c-1.08.72-2.45 1.15-4.06 1.15-3.12 0-5.77-2.11-6.71-4.95H1.29v3.1A12 12 0 0 0 12 24Z"),
    (0xFBBC05, "M5.29 14.28a7.21 7.21 0 0 1 0-4.56v-3.1H1.29a12 12 0 0 0 0 10.76l4-3.1Z"),
    (0xEA4335, "M12 4.75c1.76 0 3.34.61 4.59 1.8l3.43-3.43C17.95 1.19 15.24 0 12 0A12 12 0 0 0 1.29 6.62l4 3.1C6.23 6.86 8.88 4.75 12 4.75Z"),
