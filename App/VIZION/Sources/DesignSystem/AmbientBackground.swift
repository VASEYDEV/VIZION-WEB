        ...
        GeometryReader { proxy in
          ...
          ZStack {
            bloom(VZ.accent.opacity(0.16), diameter: size.width * 0.9)
              .position(
                x: size.width * (0.2 + 0.08 * sin(drift)),
                y: size.height * (0.18 + 0.05 * cos(drift * 0.8))
              )
            bloom(VZ.muted.opacity(0.14), diameter: size.width * 1.0)
              .position(
                x: size.width * (0.85 - 0.06 * cos(drift * 0.7)),
                y: size.height * (0.45 + 0.06 * sin(drift * 0.6))
              )
            bloom(Color(hex: 0x4B7896, alpha: 0.11), diameter: size.width * 0.7)
              .position(
                x: size.width * (0.5 + 0.1 * sin(drift * 0.5)),
                y: size.height * (0.85 - 0.04 * cos(drift))
              )
